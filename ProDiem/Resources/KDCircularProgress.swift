...
open class KDCircularProgress: UIView {
    ...
    fileprivate class KDCircularProgressViewLayer: CALayer {
        ...
        override func draw(in ctx: CGContext) {
            ...
            let point = CGPoint(x: CGFloat(size.width/2.0), y: CGFloat(size.height/2.0))
            ctx.addArc(center: point, radius: arcRadius, startAngle: 0, endAngle: CGFloat(M_PI * 2), clockwise: true)
            trackColor.set()
